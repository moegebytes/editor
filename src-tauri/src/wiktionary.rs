...
impl WiktDb {
  ...
  fn load_senses(&self, word_id: i64) -> Result<Vec<WiktSense>, WiktError> {
    let mut stmt = self
      .db
      .prepare_cached("SELECT id, gloss, tags FROM senses WHERE word_id = ?1 ORDER BY sort_order")?;
  ...
  fn load_examples(&self, sense_id: i64) -> Result<Vec<WiktExample>, WiktError> {
    let mut stmt = self
      .db
      .prepare_cached("SELECT text, english, romaji FROM examples WHERE sense_id = ?1")?;
  ...
  fn load_relations(&self, word_id: i64, sense_id: Option<i64>) -> Result<Vec<WiktRelation>, WiktError> {
    ...
    match sense_id {
      Some(sid) => {
        let mut stmt = self
          .db
          .prepare_cached("SELECT kind, term FROM relations WHERE word_id = ?1 AND sense_id = ?2")?;
        let results = stmt
          .query_map(rusqlite::params![word_id, sid], map_row)?
      ...
      None => {
        let mut stmt = self
          .db
          .prepare_cached("SELECT kind, term FROM relations WHERE word_id = ?1 AND sense_id IS NULL")?;
...
mod tests {
  ...
  fn test_db_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
      .parent()
      .unwrap()
      .join("resources/gen/wiktionary.sqlite")
  }
  ...
  fn lookup_with_relations() {
    ...
    assert!(
      sense_rels.iter().any(|r| r.kind == "coordinate_term" && r.term == "影"),
      "expected coordinate_term 影"
    );
    assert!(
      sense_rels.iter().any(|r| r.kind == "synonym" && r.term == "パンパン"),
      "expected synonym パンパン"
    );
  }
